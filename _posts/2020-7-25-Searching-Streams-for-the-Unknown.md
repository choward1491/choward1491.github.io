...
$$\mathbb{E}\left(I_X\right) = \sum_{i=1}^{n - X + 1} i \text{Pr}\left\lbrace I_{X} = i\right\rbrace \leq X \exp\left(\frac{X)}{n+1}\right) \sum_{i=1}^{n - X + 1} \left(\frac{i \exp\left(-\frac{Xi)}{n+1}\right)}{n-i+1}\right) \right)$$
